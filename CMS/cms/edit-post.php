<?php 
    ...
    //call function to get each post by ID
    $post = $fn->getPostByID( $_GET["id"] );
    
    //call function to edit a post
	$fn->editPost();
...
<!--Tiny MCE-->
	...
	<body>
		...
		<div id="wrapper" style="color: white">
			...
			<h2 class="text-center" style="color: white; letter-spacing: 5px;">Edit Post</h2>
        ...
        <!--Show error if a form is not filled-->
			...
			<form action="" method="post">
				...
				<input type='hidden' name='postID' value='<?php echo $post[ 'post_id' ]; ?>'></p>

				<p><label><b>Title</b></label><br />
				<input type='text' name='postTitle' style="width: 600px; border-radius: 5px;" value='<?php echo $post[ 'post_title' ]; ?>'></p>

				<p><label><b>Description</b></label><br />
				<textarea name='postDescription'  cols='60' rows='10'><?php  echo $post[ 'post_description' ]; ?></textarea></p>

				<p><label><b>Content</b></label><br />
				<textarea name='postContent' cols='60' rows='10'><?php echo $post[ 'post_content' ]; ?></textarea></p>

				<p><input type='submit' class="btn btn-success" name='edit' value='Edit Post'></p>
